...
ZiiPOSdownloadUrl = "https://download.ziicloud.com/programs/ziipos/ZiiLocalServerSetup(v2.6.8.9).exe"
...
def AnydeskInstallationProcess(directory):
    ...
    if (file_exists==True):
        ...
        os.system(runCMD1)
        runCMD2='cmd /c echo Ziitech123! | "C:\Program Files (x86)\AnyDesk\AnyDesk.exe" --set-password'
        os.system(runCMD2)
    else:
...
def syncDateAndTime():
    runCMD1='cmd /c w32tm /register'
    runCMD2='cmd /c net start w32time'
    runCMD3='cmd /c w32tm /resync'
    os.system(runCMD1)
    os.system(runCMD2)
    os.system(runCMD3)
...
class App:
    def __init__(self, root):
        ...
        #setting title
        root.title("ZiiPOS FB Deployment Tool V1.7.2")
        #setting window size
        width=350
        height=300
