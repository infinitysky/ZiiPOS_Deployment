...
def AccessDatabaseEngineInstallationProcess(directory):
    ...
    if (file_exists==True):
        print("AccessDatabaseEngine Ready")
        runCMD1='cmd /c C:\Ziitech\AccessDatabaseEngine.exe /Q'
        os.system(runCMD1)
    else:
...
def installZiiPOSRetail():
    ...
    version = str(response_data['data']['versionValue'])
    downloadUrl=response_data['data']['upgradeUrl']
    #versionValue=response_data['data']['versionValue']
    print(downloadUrl)
    directory ="C:\Ziitech"
    createTempFolder(directory)
    createKillScriptFile(directory)
